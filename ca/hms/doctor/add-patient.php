...
//$patage=$_POST['patage'];
$patbday = explode("/", $patbday);
$patage = (date("md", date("U", mktime(0, 0, 0, $patbday[0], $patbday[1], $patbday[2]))) > date("md")
    ? ((date("Y") - $patbday[2]) - 1)
    : (date("Y") - $patbday[2]));
$medhis=$_POST['medhis'];
$sql=mysqli_query($con,"insert into tblpatient(Docid,PatientName,PatientContno,PatientEmail,PatientGender,PatientAdd,PatientAge,PatientMedhis,PatientBday) values('$docid','$patname','$patcontact','$patemail','$gender','$pataddress','$patage','$medhis','$patbday')");
if($sql)
{
